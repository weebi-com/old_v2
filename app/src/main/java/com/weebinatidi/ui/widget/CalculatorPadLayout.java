...
package com.weebinatidi.ui.widget;
...
public class CalculatorPadLayout extends ViewGroup {
    ...
    public CalculatorPadLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        final TypedArray a = context.obtainStyledAttributes(attrs,
                new int[]{android.R.attr.rowCount, android.R.attr.columnCount, R.attr.doubleRow}, defStyle, 0);
        mRowCount = a.getInt(0, 1);
        mColumnCount = a.getInt(1, 1);
        mDoubleRow = a.getInt(2, -1);
        Log.d("CalculatorPadLayout", "Row to double = " + mDoubleRow);
        a.recycle();
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        for (int childIndex = 0; childIndex < getChildCount(); ++childIndex) {
            ...
            int childTop = 0;
            if (mDoubleRow == -1 || childIndex != mDoubleRow + 1)
                childTop = paddingTop + lp.topMargin + rowIndex * rowHeight;
            else
                childTop = paddingTop + lp.topMargin + (rowIndex + 1) * rowHeight;

            int childBottom = 0;
            if (mDoubleRow != childIndex)
                childBottom = childTop - lp.topMargin - lp.bottomMargin + rowHeight;
            else
                childBottom = childTop - lp.topMargin - lp.bottomMargin + (rowHeight * 2);
            ...
            //if (mDoubleRow != -1 && childIndex == getChildCount() - 2)
            //    return;
        }
    }
